export type RoomPlayer = { id: string; name: string; x: number; z: number; heading: number; health: number; kills: number; ammo: number[]; driving: boolean; connected: boolean };
export type Loot = { id: string; x: number; z: number; kind: "ammo" | "medkit"; taken: boolean };
