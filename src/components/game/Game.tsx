...
import { DISTRICTS, FUEL_PRICE, FUEL_STATIONS, INITIAL_STATS, VEHICLES, WEAPONS, type GameCommand, type GameController } from "@/game/config";
...
type Panel = "map" | "garage" | "armory" | "help" | "pause" | "services" | null;
  ...
  return <main className="game-shell">
    ...
    <div className="main-area">
      ...
      <section className={`world ${!started ? "world-welcome" : ""}`} aria-label="Game world">
        ...
        {started && !panel && <>
          <div className="vitals" aria-label="Player status"><span>HEALTH <strong data-testid="health">{Math.ceil(stats.health)}</strong><meter min="0" max="100" value={stats.health} aria-label="Health" /></span><span>FUEL <strong data-testid="fuel">{stats.fuel.toFixed(1)} L</strong><meter min="0" max="100" value={stats.fuel} aria-label="Fuel" /></span><button onClick={() => openPanel("services")}>WALLET <strong data-testid="credits">{stats.credits.toFixed(0)} cr</strong></button></div>
        ...
        <div className="world-bottom">
          ...
          <div className="drive-hud"><div className="speed"><strong data-testid="speed">{String(stats.speed).padStart(3, "0")}</strong><span>{stats.driving ? "KM/H" : "ON FOOT"}</span></div><div className="speed-line"><span style={{ width: `${Math.min(100, stats.speed / currentCar.speed * 100)}%` }} /></div><div className="drive-details"><span>{stats.driving ? "AUTO" : WEAPONS[weapon].name.toUpperCase()}</span><b>{stats.driving ? (stats.speed > 0 ? "D" : "N") : stats.reloading ? "RELOADING" : `${stats.ammo} / ${WEAPONS[weapon].capacity}`}</b><span>{stats.driving ? "FREE ROAM" : `${stats.hits} HITS`}</span></div></div>
        </div>
