...
const schema = z.object({ action: z.enum(["create", "join", "sync", "start", "leave", "fire", "reload", "pickup"]), code: z.string().regex(/^[A-F0-9]{8}$/).optional(), token: z.string().length(48).optional(), name: z.string().max(24).optional(), x: z.number().finite().min(-2000).max(2000).optional(), z: z.number().finite().min(-2000).max(2000).optional(), heading: z.number().finite().min(-100000).max(100000).optional(), driving: z.boolean().optional(), weapon: z.number().int().min(0).max(5).optional(), pitch: z.number().finite().min(-1.5).max(1.5).optional(), range: z.number().finite().min(0).max(300).optional() });
